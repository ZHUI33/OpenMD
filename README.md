...
| 导出长图 PNG | `Ctrl + Alt + G`   | `Cmd + Alt + G`   |
| 再次导出     | `Ctrl + Shift + E` | `Cmd + Shift + E` |
...
| 可交付导出     | 有样式/无样式 HTML、长图 PNG 与可配置分页的 PDF          |
...
- 异常退出恢复：原子备份未命名和未保存修改，恢复工作区、标签和活动标签。
- 有样式或无样式 HTML，可引用相对图片或嵌入已授权本地图片 Base64。
- 长图 PNG；A4/Letter PDF 支持主题、页边距、页眉、页脚、页码与标题分页。
- 每个文档记忆最近一次成功导出配置，并可一键再次导出。
- 浅色、深色、跟随系统与用户主题。
- 完整键盘操作、模态焦点管理、高对比度和减弱动画适配。
...
pnpm build
pnpm benchmark -- --label local
```

基准定义、文件规模和测量口径见 [benchmarks/README.md](benchmarks/README.md)。
...
- 所见即所得模式仍需为整个文档建立编辑树；多 MB 文档首次进入该模式会慢于源码模式。
- 恢复记录位于 Electron `userData/recovery`，仅用于异常退出；恢复的修改稿作为新副本打开。
